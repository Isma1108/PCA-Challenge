...
// Hay varias optimizaciones, pero sobretodo unrolling del for interno
// + unrolling SIMD Neon. 
inline unsigned int compute_triplets(unsigned int N) {
  ...
  for (m = 1; (n_end=m*m) < N; m++) {
    ...
    for (n=m+1; (n+126)*(n+126) <= n_end; n += 128) {
      ...
      uint8x16_t data5 = vld1q_u8((uint8_t*)(p+64));
      uint8x16_t data6 = vld1q_u8((uint8_t*)(p+80));
      uint8x16_t data7 = vld1q_u8((uint8_t*)(p+96));
      uint8x16_t data8 = vld1q_u8((uint8_t*)(p+112));
      
      uint8x16_t sum = vaddq_u8(data1, data2);
      uint8x16_t sum2 = vaddq_u8(data3, data4);
      uint8x16_t sum3 = vaddq_u8(data5, data6);
      uint8x16_t sum4 = vaddq_u8(data7, data8);
      
      uint8x16_t sol = vaddq_u8(vaddq_u8(sum, sum2),vaddq_u8(sum3, sum4));
      uint8_t sum_array[16];
      vst1q_u8(sum_array, sol);
